    ...
    validateChain() {
        this.app.get("/validateChain", async (req, res) => {
            ...
            if(validation){
                ...
                return res.status(500).send("There was an error while attempting validation.");
            }        
        });
    }
        ...
        this.app.get("/hash/:hash", async (req, res) => {
            if(req.params.hash) {
                ...
                if(block){
                    ...
                    return res.status(404).send("Poopy! The block couldn't be found??");
                }
            } else {
                return res.status(404).send("Block Not Found! Review the Parameters!");
            }            
            ...
            if(req.params.address) {
                ...
                try {
                    ...
                    if(stars.length > 0){
                        ...
                        return res.status(404).send("No stars registered by owner.");
                    }
                } catch (error) {
                    return res.status(500).send("An error happened!");
                }
